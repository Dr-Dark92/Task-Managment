...
TM.Store=(()=>{
 ...
 const TYPES={
  ...
  project:{prefix:'PRJ',path:['Projects','Records']},projectEvent:{prefix:'PEV',path:['Projects','Events']},projectMember:{prefix:'PMB',path:['Projects','Members']},task:{prefix:'TSK',path:['Tasks','Records']},taskEvent:{prefix:'TEV',path:['Tasks','Events']},
 ...
 async function createTask({projectId,title,description='',creatorId,assigneeId=null,priority='Normal',status='Draft',dueAt=null}){if(!projectId||!title||!creatorId)throw Error('projectId, title and creatorId required');return create('task',{projectId,title,description,creatorId,assigneeId,priority,status,dueAt})}
 ...
 return{SCHEMA,init,bootstrapSystem,create,get,list,createUser,createGroup,addMembership,createProject,addProjectEvent,addProjectMember,createTask,addTaskEvent,createEvent,scanEvents};
})();