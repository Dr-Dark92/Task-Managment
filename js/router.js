(()=>{
 ...
 const alwaysVisible=new Set(['dashboard','projects','tasks','tickets','calendar','messages','announcements','documents','kpi','dummy-data']);
 ...
 function route(page){if(page==='dummy-data'&&window.TM?.DummyData){TM.DummyData.page();return true}if(!window.TM?.Stage03)return false;if(page==='dashboard'){TM.Stage03.dashboard();return true}if(page==='projects'){TM.Stage03.projectsPage();return true}if(page==='tasks'){TM.Stage03.tasksPage();return true}return false}
